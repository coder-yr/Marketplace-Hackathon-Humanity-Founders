...
export class WorkspaceService {
  async getWorkspaceData(userId: string, role: 'buyer' | 'supplier'): Promise<WorkspaceResponse> {
    ...
    if (role === 'buyer') {
      rfqs = await Rfq.find({ buyerId: userObjectId }).sort({ createdAt: -1 }).limit(50)
      orders = await Order.find({ buyerId: userObjectId }).sort({ createdAt: -1 }).limit(50)
      quotes = await Quote.find({ buyerId: userObjectId }).sort({ createdAt: -1 }).limit(50)
    } else {
      rfqs = await Rfq.find({ status: { $in: ['Submitted', 'Viewed', 'Responded'] } }).sort({ createdAt: -1 }).limit(50)
      orders = await Order.find({ supplierId: userObjectId }).sort({ createdAt: -1 }).limit(50)
